export default class BurgerMenu {
	...
	handleTouchMove(event) {
		...
		// Вычисляем сдвиг: для движения влево разница (start - current) будет положительной
		const deltaX = this.touchStartX - currentX;

		// Ограничиваем, чтобы меню не уезжало вправо (только влево, > 0)
		const translateX = Math.max(0, deltaX);

		// Сдвигаем через transform (это производительнее, чем менять right)
